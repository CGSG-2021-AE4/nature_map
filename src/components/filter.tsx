...
export interface FilterProps {
  listId: number,
  filterId?: number,
  removeCallBack: ( filter: Filter )=>void,
}
export interface FilterState {
  filterType: FilterType,
  filterData: FilterData,
  setFilterData: ( newData: FilterData )=>void
}
...
export class Filter extends React.Component<FilterProps, FilterState> {
  ...
  constructor( props: FilterProps ) {
    super(props);
    const [filterData, setFilterData] = useFilter(props.listId, props.filterId != undefined ? props.filterId : genID(), {name: 'Name', value: 'Value'});
    this.state = {
      filterType: FilterType.Edit,
      filterData: filterData,
  ...
  render() {
    ...
    if (this.state.filterType == FilterType.Default as FilterType) {
      return (
        <div className="filter filterFrame">
          <div className="filter mainPart">
            <p className="filter text">{this.state.filterData.name + ' = ' + this.state.filterData.value}</p>
          </div>
          <input type="button" className="filter editButton" onClick={()=>{
              this.setState({ filterType: FilterType.Edit });
            }}/>
          <input type="button" className="filter deleteButton"  onClick={()=>{
              this.props.removeCallBack(this);
            }}/>
        </div>
      );
    } else if (this.state.filterType == FilterType.Edit as FilterType) {
      return (
        <div className="filter filterFrame">
          <div className="filter mainPart">
            <input type="text" className="filter nameInput" value={this.state.filterData.name} placeholder="name" onChange={e=>{
                this.setState({ filterData: {name: e.target.value, listId: this.state.filterData.listId, value: this.state.filterData.value}});
              }}/>
            <p style={{marginInline: '0.5em', padding: 0}}>=</p>
            <input type="text" className="filter valueInput" value={this.state.filterData.value} placeholder="value" onChange={e=>{
                this.setState({ filterData: {value: e.target.value, listId: this.state.filterData.listId, name: this.state.filterData.name}});
              }}/>
          </div>
          <input type="button" className="filter saveButton" onClick={()=>{
              this.state.setFilterData(this.state.filterData);
              this.setState({ filterType: FilterType.Default });
            }}/>
          <input type="button" className="filter deleteButton" onClick={()=>{
              this.props.removeCallBack(this);
            }}/>
